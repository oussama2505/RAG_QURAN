<script lang="ts">
  ...
  $: advancedOptionsText = $languageStore === 'en' 
    ? 'Advanced Options'
    : $languageStore === 'ar' 
    ? 'خيارات متقدمة'
    : 'Opciones avanzadas';
  ...
  $: translationText = $languageStore === 'en' 
    ? 'Translation'
    : $languageStore === 'ar' 
    ? 'ترجمة'
    : 'Traducción';
  ...
  $: summarizationText = $languageStore === 'en' 
    ? 'Summarize Results'
    : $languageStore === 'ar' 
    ? 'تلخيص النتائج'
    : 'Resumir resultados';
  ...
  $: summaryLengthText = $languageStore === 'en' 
  ...
  $: verseRangeText = $languageStore === 'en' 
    ? 'Verse Range'
    : $languageStore === 'ar' 
    ? 'نطاق الآيات'
    : 'Rango de versos';
...
<div class="advanced-search-container" class:rtl={$languageStore === 'ar'}>
  <form on:submit|preventDefault={handleSubmit} class="flex flex-col space-y-4">
    ...
    {#if showAdvancedOptions}
      <div class="advanced-options-panel bg-base-200 p-4 rounded-lg shadow" transition:slide>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <!-- Surah & Verse Filters -->
          <div class="form-control">
            <label class="label">
              <span class="label-text" class:arabic={$languageStore === 'ar'}>Surah</span>
            </label>
            <input 
              type="number" 
          ...
          <div class="form-control">
            <label class="label">
              <span class="label-text" class:arabic={$languageStore === 'ar'}>{verseRangeText}</span>
            </label>
          ...
          <div class="form-control">
            <label class="label">
              <span class="label-text" class:arabic={$languageStore === 'ar'}>{translationText}</span>
            </label>
            <select bind:value={translationOption} class="select select-bordered w-full">
          ...
          <div class="form-control">
            <label class="label cursor-pointer">
              <span class="label-text" class:arabic={$languageStore === 'ar'}>{summarizationText}</span> 
              <input type="checkbox" bind:checked={useSummarization} class="toggle toggle-primary" />
            </label>
          </div>
          ...
          {#if useSummarization}
            <div class="form-control">
              <label class="label">
                <span class="label-text" class:arabic={$languageStore === 'ar'}>{summaryLengthText}</span>
              </label>
              <input 
                type="number" 
            ...
            <div class="form-control">
              <label class="label">
                <span class="label-text" class:arabic={$languageStore === 'ar'}>{summaryFocusText}</span>
              </label>
              <input 
                type="text" 
